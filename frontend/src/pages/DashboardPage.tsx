...
function DashboardPage() {
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-950"><div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10"><Link to="/" className="mb-6 inline-flex items-center gap-2 text-sm font-medium text-slate-600 transition hover:text-slate-950"><ArrowLeft className="size-4" />Back to chat</Link>
    ...
    {trace && <div className="space-y-6"><TransactionHeader trace={trace} /><div className="grid gap-6 lg:grid-cols-2"><SpendSummary trace={trace} /><IntegrityPanel trace={trace} /></div>{trace.intent?.usage_mode === "reusable_budget" ? <ExecutionBranches trace={trace} /> : <div className="grid gap-4 md:grid-cols-3"><IntentCard intent={trace.intent} /><CartCard cart={trace.carts.at(-1)} /><PaymentCard payment={trace.payments.at(-1)} /></div>}<MandateChain trace={trace} /><AuditTimeline entries={trace.audit_timeline} /><TechnicalDetails trace={trace} /></div>}
  </div></main>;
}
